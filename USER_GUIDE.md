...
3. 중앙 구분선에 마우스를 올려 좌우 크기 조절 커서가 나타나는지 확인합니다.
4. 구분선을 좌우로 끌어 창 너비를 조절합니다. 조절 범위는 30:70부터 70:30까지입니다.
5. 구분선을 더블클릭하면 50:50으로 돌아갑니다.
6. 양쪽에서 다른 폴더로 이동한 뒤 **Save current paths**를 선택합니다.
7. 앱을 종료했다 다시 실행해 두 경로와 분할 비율이 복원되는지 확인합니다.
8. Google Drive 안의 동기화된 항목을 우클릭하고 Windows 11에서는 **추가 옵션 표시**를 누릅니다.
9. **구글 클라우드 URL 복사하기**를 선택하고 메모장에 붙여넣어 Drive URL인지 확인합니다.
